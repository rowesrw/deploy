...
async def aii(client: Client, message):
   ...
   try:
    ...
    chat_id = message.chat.id
    user_id = message.from_user.id if message.from_user else "A_q_lp"
    videoid = None
    link = None
...
async def play(client: Client, message):
  ...
  if not message.reply_to_message:
     ...
     if await is_served_call(client, message.chat.id):
         ...
         requester = chatname if source.views else f"[{message.from_user.first_name}](tg://user?id={message.from_user.id})"
         if message.from_user:
          if message.from_user.photo:
           ...
           ahmed = await client.get_chat("A_q_lp")
