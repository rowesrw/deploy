   ...
   try:
    ...
    if message.new_chat_members[0].username == "A_q_lp":
      try:
        ...
        pass
      return await message.reply_text(f"**♪ انضم المطور سيمو للشات  💎 .\n♪ مرحبا بك : @A_q_lp  💎 .**")
    dev = await get_dev(bot_username)
...
"مره واحد اسمو سيمو  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
@Client.on_message(filters.command(["المطور سيمو","سيمو","المبرمج"], ""))
...
async def tiillli(client, message):
  if message.from_user.username in ["A_q_lp"]:
   ...
   if message.from_user.username == "A_q_lp":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة المطور سيمو  💎 .**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @SOURCE_ELNGOM  💎 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @SOURCE_ELNGOM  💎 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in ["A_q_lp"]:
         await message.reply_text("**♪ رتبتك هي : المطور سيمو  💎 .**")
         return
